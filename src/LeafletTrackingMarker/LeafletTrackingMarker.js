...
const computeBearing = (previousPosition = defaultPosition, nexPosition) => {
  let bearing = Math.atan2(
    nexPosition[0] - previousPosition[0],
    nexPosition[1] - previousPosition[1]
  );
...
const createMarker = (
  { position, previousPosition, rotationAngle, ...options },
  ctx
) => {


  const bearingAngle =
    rotationAngle ?? computeBearing(previousPosition, position);
